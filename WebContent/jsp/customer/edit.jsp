...
<BODY>
	<FORM id=form1 name=form1
		action="${pageContext.request.contextPath }/customer_upd.action"
		method=post>
...
<td>客户级别 ：</td>
<td>
<select name="dictCusLevel.dno">

<c:forEach var="dict" items="${listLevel }">
	<option value="${dict.dno }" <c:if test="${dict.dno == customer.dictCusLevel.dno}">selected</c:if> > ${dict.dname }</option>
	</c:forEach>
</select>
</td>
							</TR>
							...
							<TR>
								...
								<td>
								<INPUT class=textbox id=sChannel2
														style="WIDTH: 180px" maxLength=50 name="cust_industry" value="${customer.cust_industry }">
								</td>
							</TR>
							<TR>
